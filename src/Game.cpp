...
int Game::connect()
{
    std::cout << fmt::format("Connection {host}:{port}({uid})",
                             fmt::arg("uid", connection.uid), fmt::arg("host", connection.host), fmt::arg("port", connection.port))
              << std::endl;
    if (connection)
    {
        ipcon_create(&ipcon);
...
void main_loop(void* data)
{
    ...
    b2Vec2 frog_screen_position = game->world2screen(game->frog->getPosition());
    ...
    game->frog->render(renderer);
    game->pipe->render(renderer);

    SDL_RenderPresent(renderer);
    uint32_t currTime = SDL_GetTicks();
    double elapsedTime = (currTime - startTime) / 1000.0; // Convert to seconds.
    game->world.update(elapsedTime);
    game->pipe->update(elapsedTime);
}
...
int Game::loop()
{
    ...
    emscripten_set_main_loop_arg(main_loop, this, -1, 1);
